 ...
 *
 */
__global__ void applySimpleLinearBlurFilter(uchar *d_r, uchar *d_g, uchar *d_b)
{
    ...
    if(threadId < num_image_pixels)
    {
        ...
        for (int dr = -1; dr <= 1; dr++) {
            for (int dc = -1; dc <= 1; dc++) {
                ...
                if (nr >= 0 && nr < d_rows && nc >= 0 && nc < d_columns) {
                    int neighbor = nr * d_columns + nc;
                    sum_r += d_r[neighbor];
                    sum_g += d_g[neighbor];
                    sum_b += d_b[neighbor];
                    count++;
        ...
        d_r[threadId] = (uchar)(sum_r/count);
        d_g[threadId] = (uchar)(sum_g/count);
        d_b[threadId] = (uchar)(sum_b/count);
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, int rows, int columns, int threadsPerBlock)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to launch applySimpleLinearBlurFilter kernel (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
}
