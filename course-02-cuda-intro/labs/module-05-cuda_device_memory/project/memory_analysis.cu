...
__host__ void executeKernel(int *d_input, int numElements, int threadsPerBlock, std::string kernelType)
{
    ...
    } else if (!strcmp(kernelType.c_str(), "shared"))
    {
        ...
        sharedMemoryKernel<<<1,threadsPerBlock, int_array_size>>>(int numElements); // you will need to fill in function arguments appropriately, also feel free to change the number of blocks
    } else {
