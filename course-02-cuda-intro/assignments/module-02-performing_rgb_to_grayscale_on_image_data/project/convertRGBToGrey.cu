...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    int pixels = d_rows * d_columns;
    int totalThreads = (gridDim.x * gridDim.y * gridDim.z) *
                       (blockDim.x * blockDim.y * blockDim.z);

    for (int i = tid; i < pixels; i += totalThreads)
    {
        unsigned int r = d_r[i];
        unsigned int g = d_g[i];
        unsigned int b = d_b[i];
        
        // OpenCV IMREAD_GRAYSCALE와 동일한 ITU-R BT.601 공식
        // 정수 연산으로 최적화: 0.299≈77/256, 0.587≈150/256, 0.114≈29/256
        d_gray[i] = (uchar)((77 * r + 150 * g + 29 * b) >> 8);
    }
}
