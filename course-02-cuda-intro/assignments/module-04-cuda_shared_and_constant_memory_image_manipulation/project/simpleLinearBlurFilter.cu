 ...
 *
 */
__global__ void applySimpleLinearBlurFilter(uchar *r, uchar *g, uchar *b, uchar * r_out, uchar * g_out, uchar * b_out)
{
    ...
    if(threadId < num_image_pixels) {
        ...
        r_out[threadId] = (uchar)(sum_r/count);
        g_out[threadId] = (uchar)(sum_g/count);
        b_out[threadId] = (uchar)(sum_b/count);
    }
}
...
__host__ void executeKernel(uchar *r, uchar *g, uchar *b, uchar *r_out, uchar *g_out, uchar *b_out, int rows, int columns, int threadsPerBlock)
{
    cout << "Executing kernel\n";
    //Launch the convert CUDA Kernel
    int blocksPerGrid = (rows * columns) / threadsPerBlock;

    applySimpleLinearBlurFilter<<<blocksPerGrid, threadsPerBlock>>>(r, g, b, r_out, g_out, b_out);
...
int main(int argc, char *argv[])
{
    ...
    try 
    {
        ...
        uchar *r_out = (uchar *)malloc(sizeof(uchar) * rows * columns);
        uchar *g_out = (uchar *)malloc(sizeof(uchar) * rows * columns);
        uchar *b_out = (uchar *)malloc(sizeof(uchar) * rows * columns);

        executeKernel(r, g, b, r_out, g_out, b_out, rows, columns, threadsPerBlock);
        ...
        for(int y = 0; y < rows; ++y)
        {
            for(int x = 0; x < columns; ++x)
            {
                colorImage.at<Vec3b>(y,x) = Vec3b(b_out[y*columns+x], g_out[y*columns+x], r_out[y*columns+x]);
            }
        }
        ...
        float scaledMeanDifferencePercentage = compareColorImages(r_out, g_out, b_out, test_r, test_g, test_b, rows, columns) * 100;
