...
__global__ void globalMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        for (int idx = threadId; idx < numElements; idx += numElements/numThreads) {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            int new_value = input[idx] - 1;
            input[idx] = new_value;

            if (input[idx] == constant_search_value) {
...
__global__ void sharedMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        for (int idx = threadId; idx < numElements; idx += numElements/numThreads) {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            int new_value = sharedInput[idx];
            sharedInput[idx] = new_value - 1;
            if (sharedInput[idx] == constant_search_value) {
                // atomicAdd(totalFound, 1);
            }
...
__global__ void constantMemorySearch(int totalFound)
{
    ...
    {
        // Create a for loop that handles the fact that each thread needs to search for n values in input
        for (int idx = threadId; idx < constant_num_elements; idx += constant_num_threads) {
            // Decrement the input array at index i by 1, update in memory prior to testing if it is equal to constant_search_value
            int new_value = constant_input[idx] - 1;

            if (new_value == constant_search_value) {
                // atomicAdd(totalFound, 1);
            }
...
__host__ void copyFromHostToDevice(std::string kernelType, int *input, int numElements, int numThreads, int *d_input)
{
    ...
    if(!strcmp(kernelType.c_str(), "constant"))
    {
        ...
        err = cudaMemcpyToSymbol(constant_input, input, size, 0);
        if (err != cudaSuccess)
        {
            fprintf(stderr, "Failed to copy array input from host to constant memory (error code %s)!\n", cudaGetErrorString(err));
            exit(EXIT_FAILURE);
        }
        ...
        if (err != cudaSuccess)
        {
            fprintf(stderr, "Failed to copy the number of elements from host to constant memory (error code %s)!\n", cudaGetErrorString(err));
            exit(EXIT_FAILURE);
        }
        ...
        if (err != cudaSuccess)
        {
            fprintf(stderr, "Failed to copy the threadSpan from host to constant memory (error code %s)!\n", cudaGetErrorString(err));
            exit(EXIT_FAILURE);
        }
        ...
        if (err != cudaSuccess)
        {
            fprintf(stderr, "Failed to copy the numThreads from host to constant memory (error code %s)!\n", cudaGetErrorString(err));
            exit(EXIT_FAILURE);
        }
    ...
    } else 
    {
        ...
        err = cudaMemcpy(d_input, input, numElements * sizeof(int), cudaMemcpyHostToDevice);
