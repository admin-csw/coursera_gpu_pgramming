...
std::tuple <long* ,long* ,dim3* ,dim3*> allocateMemory(long* data, int numElements, dim3 threadsPerBlock, dim3 blocksPerGrid)
{
    //
...
__host__ long* mergesort(long* data, long size, dim3 threadsPerBlock, dim3 blocksPerGrid) {
    ...
    // Initialize timing metrics variable(s). The implementation of this is up to you
    ...
    // calculate and print to stdout kernel execution time
...
__global__ void gpu_mergesort(long* source, long* dest, long size, long width, long slices, dim3* threads, dim3* blocks) {
    ...
    // nitialize 3 long variables start, middle, and end
