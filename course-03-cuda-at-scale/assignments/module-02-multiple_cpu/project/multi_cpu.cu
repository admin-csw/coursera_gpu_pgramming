...
__host__ void deallocateMemory(int *h_a, int *h_b, int *h_c, int *d_a, int *d_b)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to free constant vector c (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
}
...
__host__ void performMultiCPUIteration()
{
    ...
    ofstream output_lock_a_file;
	output_lock_a_file.open("./output_a.lock");
    ofstream output_lock_b_file;
	output_lock_b_file.open("./output_b.lock");
