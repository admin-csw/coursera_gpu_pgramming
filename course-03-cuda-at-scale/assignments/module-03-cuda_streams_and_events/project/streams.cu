...
// Reads an integer seed from stdin in the range [0, 255].
// Keeps prompting until the user provides a valid value.
// Note: This is a host-only function (do not call from device code).
__host__ int readSeed0to255()
{
    /*
    int s = -1;

    while (true) {
        std::printf("Enter seed s (0~255): ");
        std::fflush(stdout);

        // Accept only an integer in [0, 255]
        if (std::scanf("%d", &s) == 1 && (0 <= s && s <= 255)) {
            return s;
        }

        // Clear invalid input (e.g., non-numeric characters) from stdin
        int ch;
...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    int s = readSeed0to255();
    float random_max = 255.0f;
    srand(s);
    float x = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/random_max));


    kernelA1<<<threadsPerBlock, blocksPerGrid, 0, stream2>>>(dev_mem, num_elements, x);
    kernelB1<<<threadsPerBlock, blocksPerGrid, 0, stream3>>>(dev_mem, num_elements);
    ...
    kernelA2<<<threadsPerBlock, blocksPerGrid, 0, stream4>>>(dev_mem, num_elements, x);
    kernelB2<<<threadsPerBlock, blocksPerGrid, 0, stream5>>>(dev_mem, num_elements);
...
__host__ float * runStreamsBlockingKernel2StreamsNaive(float *host_mem, int num_elements)
{
    ...
    kernelA1<<<threadsPerBlock, blocksPerGrid, 0, stream2>>>(dev_mem, num_elements, x);
    kernelB1<<<threadsPerBlock, blocksPerGrid, 0, stream2>>>(dev_mem, num_elements);
    ...
    kernelA2<<<threadsPerBlock, blocksPerGrid, 0, stream3>>>(dev_mem, num_elements, x);
    kernelB2<<<threadsPerBlock, blocksPerGrid, 0, stream3>>>(dev_mem, num_elements);
...
__host__ float * runStreamsBlockingKernel2StreamsOptimal(float *host_mem, int num_elements)
{
    ...
    kernelA2<<<threadsPerBlock, blocksPerGrid, 0, stream2>>>(dev_mem, num_elements, x);
    kernelB2<<<threadsPerBlock, blocksPerGrid, 0, stream3>>>(dev_mem, num_elements);
