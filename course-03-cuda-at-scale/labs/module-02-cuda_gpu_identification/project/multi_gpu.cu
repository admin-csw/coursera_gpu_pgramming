...
int main() {
  ...
  int maxProcessorCount = -1;
  int maxProcessorCountDeviceNumber = -1;
  ...
  for (int i = 0; i < nDevices; i++) {
    ...
    if (prop.multiProcessorCount > maxProcessorCount) {
      maxProcessorCount = prop.multiProcessorCount;
      maxProcessorCountDeviceNumber = i;
    }
  }

  // Create logic to actually choose the device based on one or more device properties
  currentChosenDeviceNumber = maxProcessorCountDeviceNumber;
